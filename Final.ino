...
/* Includes ---------------------------------------------------------------- */
#include <TableTennisMotion_inferencing.h>
...
#define pinBuzzer 3
void buzzer(int pred_index);
void tocar(int vezes);
...
void tocar(int vezes){
  int duracao = 400/vezes; // Duração de 2 segundos
  int melodia = 300 + 200*vezes;
  for (int i = 0; i < vezes; i++) {
    tone(pinBuzzer, melodia, duracao);
    delay(duracao);
    noTone(pinBuzzer);
    delay(duracao);
  }
